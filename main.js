                    ...
                    if (FORMPROCESSOR_POST_TO_CANVAS) {
                        if ('canvasasmtid' in parsedjsonobj) {
                            ...
                            for (i = 0; i < CANVAS_STUDENTS.length; i++) { // for multiple sections
                                ...
                                if (user_id.length > 0 && asmtidx >= 0) {
                                    let gradeurl = "/api/v1/courses/"+CANVAS_COURSE_ID[i]+"/assignments/" + asmtid[asmtidx] + "/submissions/update_grades?grade_data["+user_id+"][posted_grade]="+canvaspoints+"&grade_data["+user_id+"][excuse]=true";
